    ...
    <div>
      ...
      <div className="relative">
        <input
          ...
          className="w-full px-4 py-3 bg-slate-700 border border-slate-600 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-purple-500 transition-colors"
        />
        <button
          type="button"
      ...
      {value && (
        <div className="mt-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className={`text-xs font-semibold ${strength.color === 'bg-green-500' ? 'text-green-400' : strength.color === 'bg-yellow-500' ? 'text-yellow-400' : strength.color === 'bg-orange-500' ? 'text-orange-400' : 'text-red-400'}`}>
              {strength.text}
            </span>
          </div>
